...
const RentaVehiculo = ({ navigation }) => {
    ...
    const onSearch = async (data) => {
      ...
      try {
          const response = await axios.get(`http://192.168.0.2:3000/api/carros/${data.placa}`);
  ...
  const onUpdate=async(data) => {
    ...
    try {
      const response2 = await axios.put(`http://192.168.0.2:3000/api/carros/${_id}`,{
      placa,
      marca,
      valordiario,
      estado:false
    ...
    const onSubmit =async (data) => {
      ...
      try {
        const response = await axios.post(`http://192.168.0.2:3000/api/rentas
      `, {
    ...
    {/*Boton*/}
    <Button style={{marginTop:10}} mode="contained" textColor='white'  buttonColor='purple' onPress={handleSubmit(onSearch)}>Rentar</Button>
